...
template <typename PointType>
typename KD_TREE<PointType>::PointVector KD_TREE<PointType>::Snapshot()
{
    PointVector storage;

    // Participate in the same reader exclusion used by nearest-neighbour
    // searches.  The asynchronous rebuild thread waits for this counter to
    // reach zero before replacing a subtree and deleting the old nodes.
    pthread_mutex_lock(&search_flag_mutex);
    while (search_mutex_counter == -1)
    {
        pthread_mutex_unlock(&search_flag_mutex);
