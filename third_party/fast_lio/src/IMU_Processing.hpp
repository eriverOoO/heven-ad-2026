...
class ImuProcess
{
 public:
  ...
  void set_point_undistortion_enabled(bool enabled) {
    point_undistortion_enabled_ = enabled;
  }
 ...
 private:
  ...
  bool   point_undistortion_enabled_ = true;
};
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  // MORAI emits a completed 360-degree cloud from one Unity-frame pose.  We
  // still propagate the IMU state to the snapshot timestamp, but must not
  // warp points using synthetic azimuth/firing offsets.
  if (!point_undistortion_enabled_) return;
...
void ImuProcess::Process(const MeasureGroup &meas,  esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init_)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT)
    {
      ...
      // The upstream optional file logger is intentionally disabled in this vendor port.
    }
