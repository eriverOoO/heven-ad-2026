#ifndef AD_LIDAR_PERCEPTION__OCCUPANCY_GRID__EXACT_STAMP_PAIRER_HPP_
#define AD_LIDAR_PERCEPTION__OCCUPANCY_GRID__EXACT_STAMP_PAIRER_HPP_
...
public:
  ...
  explicit ExactStampPairer(std::size_t maximum_pending_messages)
...
private:
  ...
  public:
    void put(
      std::int64_t stamp_ns, Value value,
      std::size_t maximum_pending_messages)
    {
      ...
      while (values_.size() > maximum_pending_messages) {
        const auto oldest = arrival_order_.front();
    ...
    Value take(std::int64_t stamp_ns)
    {
  ...
  static void validate_stamp(std::int64_t stamp_ns)
  {
    if (stamp_ns <= 0) {
      throw std::invalid_argument(
  ...
  std::optional<Pair> match(std::int64_t stamp_ns)
  {
    ...
    {
      // A complete exact-stamp pair in the past is the only unambiguous
      // evidence available here that simulator time started a new epoch.
      left_.clear();
      right_.clear();
    }
...
#endif  // AD_LIDAR_PERCEPTION__OCCUPANCY_GRID__EXACT_STAMP_PAIRER_HPP_
