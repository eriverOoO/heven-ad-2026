...
def _create_node(context):
    ...
    if not data_dir or not path_file or not corridor_file:
        raise RuntimeError(
            "cut-in risk requires data_dir, path_file, and route_corridor_file"
        )
    ...
    try:
        ...
        raise RuntimeError(f"cut-in risk requires global path: {path}") from error
    return [
        Node(
            package="ad_planner",
