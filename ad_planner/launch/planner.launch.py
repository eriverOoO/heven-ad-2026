...
def _create_planner_node(context):
    ...
    highway_merge_reference_path_override = LaunchConfiguration(
        "enable_highway_merge_reference_path", default=""
    ).perform(context).strip().lower()
    if highway_merge_reference_path_override:
        if highway_merge_reference_path_override not in {"true", "false"}:
            raise RuntimeError(
                "enable_highway_merge_reference_path must be empty, true, or false"
            )
        overrides["enable_highway_merge_reference_path"] = (
            highway_merge_reference_path_override == "true"
        )
...
def _create_cut_in_risk_node(context, force=False):
    ...
    if not isinstance(selected_path, str) or not selected_path:
        raise RuntimeError("cut-in risk requires a configured global path")
    if not isinstance(selected_corridor, str) or not selected_corridor:
        raise RuntimeError("cut-in risk requires route_corridor_file")
    package_share = get_package_share_directory("ad_planner")
    return Node(
        package="ad_planner",
        ...
        output="screen",
        parameters=[
            os.path.join(package_share, "config", "cut_in_risk.yaml"),
            {
                "data_dir": data_dir,
                "route_corridor_file": selected_corridor,
                "route_corridor.expected_global_path_sha256": (
...
def _create_planner_actions(context):
    ...
    return [mppi_runtime, *planning_nodes]
...
def generate_launch_description():
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "enable_highway_merge_reference_path", default_value=""
            ),
