"""Live ad_planner replay proving the opt-in highway merge response integration.

The planner is driven into the FollowGlobalPath branch (perception disabled) on a
straight fixture route with an ego reporting 8 m/s. Only the
HighwayMergeGapResponse input varies:

  * nothing / inactive        -> no constraint, /ad/planner/highway_merge_speed_limit
                                 == -1, merge_authorized == False, governed target
                                 == baseline
  * MERGE_READY (active)       -> no highway-merge cap (limit == -1), governed target
                                 byte-identical to the no-response baseline, but
                                 merge_authorized flips to True
  * WAIT (active)              -> cap == ego_speed * sqrt(available / comfortable_stop)
                                 (the response's own comfortable-stop envelope);
                                 governed target drops to that cap, still < baseline;
                                 merge_authorized False
  * HOLD (active)              -> cap 0.0, governed target 0.0, command flips to
                                 brake; merge_authorized False
  * wrong merge zone           -> ignored wholesale: limit == -1, merge_authorized
                                 False even though the action is MERGE_READY
  * stale (> max age)          -> constraint drops, merge_authorized False,
                                 baseline target returns
...
LIMIT_TOPIC = "/ad/planner/highway_merge_speed_limit"
...
# WAIT facts: 40 m to the merge reference -> 34 m available after the 6 m
# standoff. A genuine WAIT (available > comfortable stop distance) whose
# comfortable-stop cap (~11.06 m/s) is well below the 16.25 m/s cruise target
# and above the 8 m/s ego speed, so the governed longitudinal target drops
# without commanding a stop.
WAIT_AVAILABLE_M = 34.0
WAIT_COMFORTABLE_STOP_M = EGO_SPEED_MPS * EGO_SPEED_MPS / (2.0 * COMFORTABLE_DECEL_MPS2)
WAIT_EXPECTED_CAP = EGO_SPEED_MPS * math.sqrt(WAIT_AVAILABLE_M / WAIT_COMFORTABLE_STOP_M)
# HOLD facts: the comfortable stopping margin has been consumed.
...
def _build_fixture():
    tmp = tempfile.TemporaryDirectory(prefix="ad_planner_highway_merge_constraint_")
    root = Path(tmp.name)
    data_dir = root / "data"
    ...
    return LaunchDescription(
        [
            ...
            SetEnvironmentVariable(
                "ROS_LOG_DIR", "/tmp/heven_planner_highway_merge_constraint_ros_log"
            ),
            LaunchNode(
                package="ad_planner",
                ...
                parameters=[
                    str(_COMMON_YAML),
                    {
                        ...
                        "highway_merge_response_max_age_s": MAX_AGE_S,
                    },
                ],
            ),
...
def _response(action, active, available_m=WAIT_AVAILABLE_M, zone=EXPECTED_ZONE):
    message = HighwayMergeGapResponse()
    message.action = action
    message.active = active
    message.reason = HighwayMergeGapResponse.REASON_NONE
    message.merge_zone_id = zone
    message.ego_speed_mps = EGO_SPEED_MPS
    message.ego_route_distance_to_merge_m = float(available_m + MERGE_STANDOFF_M)
    message.available_distance_m = float(available_m)
    message.comfortable_stop_distance_m = float(WAIT_COMFORTABLE_STOP_M)
    message.ego_merge_timing_valid = True
    return message


def _merge_ready():
    return _response(HighwayMergeGapResponse.ACTION_MERGE_READY, True)


def _wait():
    return _response(HighwayMergeGapResponse.ACTION_WAIT, True)


def _hold():
    return _response(
        HighwayMergeGapResponse.ACTION_HOLD, True, available_m=HOLD_AVAILABLE_M
    )


def _inactive():
    return _response(HighwayMergeGapResponse.ACTION_MERGE_READY, False)


def _wrong_zone_ready():
    return _response(
        HighwayMergeGapResponse.ACTION_MERGE_READY, True, zone="some_other_zone"
    )


class TestPlannerHighwayMergeConstraint(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        rclpy.init()
        cls.node = Node("planner_highway_merge_constraint_probe")
        cls.commands = []
        cls.statuses = []
        cls.limits = []
        cls.authorized = []
        ...
        cls.node.create_subscription(
            Bool, AUTHORIZED_TOPIC, lambda m: cls.authorized.append(m), _QOS_1_RELIABLE
        )
        cls.node.create_subscription(
    ...
    def _recent_limit(self):
        self.assertTrue(self.limits, "no highway_merge_speed_limit sample seen")
        return self.limits[-1].data

    def _recent_authorized(self):
        self.assertTrue(self.authorized, "no highway_merge_authorized sample seen")
        return self.authorized[-1].data
    ...
    def test_highway_merge_constraint_and_authorization(self):
        ...
        # 1. No response yet: no constraint, no authorization, cruise target.
        self._spin(3.0)
        ...
        self.assertFalse(self._recent_authorized())
        self.assertGreater(
            baseline_target, EGO_SPEED_MPS + 2.0, "expected an unconstrained cruise target"
        )

        # 2. Inactive MERGE_READY enum is a true no-op and never authorizes.
        self._spin(2.0, response=_inactive())
        self.assertEqual(self._recent_limit(), -1.0)
        self.assertFalse(self._recent_authorized())
        self.assertAlmostEqual(self._recent_target(), baseline_target, delta=0.3)

        # 3. MERGE_READY: no highway-merge cap (governed target byte-identical to
        #    the no-response baseline), but merge_authorized flips to True.
        self._spin(3.0, response=_merge_ready())
        self.assertEqual(self._recent_limit(), -1.0)
        self.assertTrue(self._recent_authorized())
        ready_target = self._recent_target()
        ready_steering = self._mean_abs_steering()
        self.assertAlmostEqual(ready_target, baseline_target, delta=0.3)

        # 4. WAIT caps the governed target at the response's comfortable-stop
        #    envelope (~11.06 m/s), below the cruise target and above the 8 m/s
        #    ego speed. Authorization drops to False immediately.
        self._spin(3.0, response=_wait())
        self.assertAlmostEqual(self._recent_limit(), WAIT_EXPECTED_CAP, places=2)
        self.assertGreater(WAIT_EXPECTED_CAP, EGO_SPEED_MPS)
        self.assertFalse(self._recent_authorized())
        wait_target = self._recent_target()
        wait_steering = self._mean_abs_steering()
        self.assertAlmostEqual(wait_target, WAIT_EXPECTED_CAP, delta=0.5)
        self.assertLess(wait_target, baseline_target - 1.0)
        ...
        # 6. A MERGE_READY for a different merge zone is ignored wholesale.
        self._spin(3.0, response=_wrong_zone_ready())
        self.assertEqual(self._recent_limit(), -1.0)
        self.assertFalse(self._recent_authorized())
        self.assertAlmostEqual(self._recent_target(), baseline_target, delta=0.5)

        # 7. Re-arm MERGE_READY, then stop publishing: authorization and the cap
        #    both expire. No latch.
        self._spin(2.0, response=_merge_ready())
        self.assertTrue(self._recent_authorized())
        ...
        # No highway-merge state ever raised the target above the nominal cruise.
        for label, value in (
            ("merge_ready", ready_target),
            ("wait", wait_target),
            ("hold", hold_target),
        ):
        ...
        Path("/tmp/heven_planner_highway_merge_constraint_result.json").write_text(
            json.dumps(
                {
                    "baseline_target": baseline_target,
                    "merge_ready_target": ready_target,
                    "wait_target": wait_target,
                    "wait_cap_mps": WAIT_EXPECTED_CAP,
...
class TestPlannerHighwayMergeConstraintShutdown(unittest.TestCase):
