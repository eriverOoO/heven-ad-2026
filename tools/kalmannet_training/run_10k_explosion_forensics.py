#!/usr/bin/env python3
"""Bounded (<=12 epoch) forensic replay of the real AV2 Scale-Up v2 10k
GENERIC-ROBUST seed-0 run, using the exact frozen dataset/split/config
and the PR #56 safe-step guard. Analysis run only -- never trains to
completion, never starts a new seed, never touches architecture/
hyperparameters, never evaluates official AV2 VAL.
...
OUTPUT = Path.home() / "datasets/av2/checkpoints/kalmannet_10k_explosion_forensics_seed0.json"
...
def main():
    ...
    print(f"starting bounded forensic search: seed={SEED} max_epochs={MAX_EPOCHS} "
          f"ring_buffer_size={RING_BUFFER_SIZE}", file=sys.stderr, flush=True)
