...
def parse():
    ...
    p.add_argument('--thresholds',type=float,nargs='+',default=[.7,.5,.3,.1]); p.add_argument('--score',type=float,default=.1); return p.parse_args()
...
def main():
    ...
    for t in a.thresholds:
        ...
        for idx,(record,g) in enumerate(zip(records,gt)):
            items=greedy_class_agnostic_nms(score_filter([det(x) for x in record['detections']],a.score),t)
        ...
        mot=evaluate(gt_seq,trk_seq,threshold_m=2.0); mot.update({'secondary_nms_iou_threshold':t,'score_threshold':a.score,'frames':len(frames),'detections':sum(len(f['detections']) for f in frames),'stage_provenance':'secondary NMS applied to historical OpenPCDet post-NMS output'})
